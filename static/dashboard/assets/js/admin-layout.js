document.addEventListener('DOMContentLoaded', function () {
  ...
  var productAccessReady = true;
  if (typeof cfg.storeProfileComplete !== 'undefined') {
    productAccessReady = cfg.storeProfileComplete !== false;
  } else {
    var productAccessReadyAttr = app.getAttribute('data-product-access-ready');
    if (productAccessReadyAttr === 'false') {
      productAccessReady = false;
    }
  }
  ...
  function buildSidebarItem(item) {
    ...
    var isDisabled = role === 'seller' && item.requiresProductAccess && !productAccessReady;
    var disabledClass = isDisabled ? ' opacity-50 pointer-events-none' : '';
    var disabledAttr = isDisabled ? ' onclick="return false;" title="Selesaikan tutorial toko terlebih dahulu untuk membuka produk."' : '';
  ...
  var roleName = cfg.userRoleDisplay || (role === 'admin' ? 'Administrator' : 'Penjual');
        ...
        '<div class="relative ml-2">' +
          ...
          '<div id="avatar-dropdown" class="hidden absolute right-0 top-full mt-3 w-64 bg-white rounded-xl shadow-xl ring-1 ring-gray-100 py-2 z-50">' +
            ...
            '<div class="py-1">' +
              ...
              '<a href="' + (urls.settings || '#') + '" class="flex items-center gap-3 px-5 py-3 text-[15px] text-gray-700 hover:bg-gray-50 transition-300">' + svgSettings + ' Pengaturan</a>' +
            '</div>' +
            '<div class="border-t border-gray-100 pt-1">' +
              '<a href="' + logoutUrl + '" class="flex items-center gap-3 px-5 py-3 text-[15px] text-gray-700 hover:bg-gray-50 transition-300">' + svgLogout + ' Keluar</a>' +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
